...
def main():
    ...
    pf["Holding_Days"] = (today - pf["Date_of_Purchase"]).dt.days

    pf["Exit_Load_Impact_%"] = 0.0

    mask_exit = pf["Will_Exit_Load_Apply"].astype(str).str.upper() == "Y"

    # ----- Period 1 -----
    p1_days = (
        pf.loc[mask_exit, "Exit_Load_Period__1"]
        .astype(str)
        .str.extract(r"(\d+)")[0]
        .astype(float)
    )

    p1_pct = pf.loc[mask_exit, "Exit_Load_PCT__1"].fillna(0)

    # ----- Period 2 (optional) -----
    has_p2 = (
        "Exit_Load_Period__2" in pf.columns and
        "Exit_Load_PCT__2" in pf.columns
    )

    if has_p2:
        p2_days = (
            pf.loc[mask_exit, "Exit_Load_Period__2"]
            .astype(str)
            .str.extract(r"(\d+)")[0]
            .astype(float)
        )
        p2_pct = pf.loc[mask_exit, "Exit_Load_PCT__2"].fillna(0)
    ...
    pf["Exit_Load_Amount"] = (
        pf["Current_Value"] * pf["Exit_Load_Impact_%"] / 100
    ).round(2)
    ...
    pf["IRR"] = pf.apply(
        lambda r: xirr([
            (r["Date_of_Purchase"], -r["Total_Purchase_Value"]),
            (datetime.today(), r["Current_Value"])
        ]),
        axis=1
    )
