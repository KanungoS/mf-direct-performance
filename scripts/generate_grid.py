...
def load_nav_history(code):
    ...
    if "data" not in data:
        raise ValueError(f"No NAV data for Scheme Code {code}")
    ...
    # Rename date-like columns to 'date'
    ...
    if "date" not in df.columns:
        raise ValueError("NAV history is missing a valid date column")
...
def main():
    ...
    for idx, row in pf.iterrows():
        ...
        # nav_col = [c for c in history.columns if c.lower() in ("nav", "nav_value", "value")]

	if nav_col:
    	    latest_nav = history[nav_col[0]].iloc[-1]
	else:
    	    print(f"⚠ No NAV column found for {code}, skipping.")
    	    return None    # or continue gracefully
